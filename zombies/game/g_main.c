...
void G_FixPlayerGuns()
{
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) 
	{
		...
		if( !cl->hasHotPotato )
		{
			cl->ps.stats[STAT_WEAPONS] = ( 1 << WP_SHOTGUN );
			cl->ps.ammo[WP_SHOTGUN] = 999;
			cl->ps.weapon = WP_SHOTGUN;
			cl->ps.weaponstate = WEAPON_READY;
		}
		else
		{
			cl->ps.stats[STAT_WEAPONS] = ( 1 << WP_GAUNTLET );
			cl->ps.ammo[WP_SHOTGUN] = 0;
...
qboolean G_PlayerHasHotPotato()
{
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) 
	{
		...
		if( cl->hasHotPotato || cl->hasBeenKilledByPotato )
		{
			return qtrue;
		}
	}
...
qboolean G_GivePlayerHotPotato()
{
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) 
	{
		...
		{
			trap_SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " has been infected.\n\"", cl->pers.netname));
			trap_SendServerCommand( cl->ps.clientNum, va("cp \"" S_COLOR_WHITE "You have been " S_COLOR_RED "infected" S_COLOR_WHITE "!\n\"", cl->pers.netname));
...
//	trap_SendServerCommand( -1, va("cp \"num human: %d\n\"", numHumans));
	if ( numHumans == 0 )
		lastClientID = 0;
